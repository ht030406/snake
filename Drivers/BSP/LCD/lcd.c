...
/******************************************显示一个点*****************************************/
/*
    以左上角为坐标值，画十条线，线的长度也为10，由于算上了初始点，所以线的长度为y1+9。
*/
void position(uint16_t x,uint16_t y,uint16_t color)
{
    uint16_t x1 = x*10+75;
    uint16_t y1 = y*10+50;
    lcd_fill(x1,y1,x1+9,y1+9,color);
}

/*****************************************蛇身显示不一样，有节点作为区分************************/
void position_body(uint16_t x,uint16_t y,uint16_t color)
{
    uint16_t x1 = x*10+75;
    uint16_t y1 = y*10+50;
    lcd_fill(x1+1,y1+1,x1+8,y1+8,color);
    lcd_draw_rectangle(x1,y1,x1+9,y1+9,WHITE);
    /*lcd_draw_line(x1+9,y1,x1+9,y1+9,WHITE);
    lcd_draw_line(x1,y1,x1+9,y1,WHITE);
    lcd_draw_line(x1,y1,x1,y1+9,WHITE);
    lcd_draw_line(x1,y1+9,x1+9,y1+9,WHITE);*/
}

/******************************************清除一个点******************************************/
void position_clear(uint16_t x,uint16_t y)
{
    uint16_t x1 = x*10+75;
    uint16_t y1 = y*10+50;
    lcd_fill(x1,y1,x1+9,y1+9,LGRAY);
}
